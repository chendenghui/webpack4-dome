...
// 字母和数字
export const validateOrder = (rule, value, callback) => {
  if (validateEmpty(rule, value, callback)) {
    return;
  } else if
  (!(/^[a-zA-Z0-9]+$/).test(value)) {
    rule.message = '只允许输入数字和字母！';
    callback('只允许输入数字和字母！');
  } else {
    callback();
  }
...
// 立项单号
export const validateOrderProject = (rule, value, callback) => {
  if (validateEmpty(rule, value, callback)) {
    return;
  } else if
  (!(/^[a-zA-Z0-9-]+$/).test(value)) {
    rule.message = '只允许输入数字，字母和"-"！';
    callback('请输入正确单号！');
  } else if (!(/[a-zA-Z0-9]{3}-[a-zA-Z0-9]{8}-[a-zA-Z0-9]{3}/).test(value)) {
    rule.message = '请输入正确格式，如"LXSQ-20180101-001，LXCG-20180101-001"';
    callback('请输入正确单号！');
  } else {
    callback();
  }
