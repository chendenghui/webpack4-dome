 ...
 *    router.close(); // 关闭页面
 *    // 非页面地址
 *    router.buildUrl('/scm/supplier/api/export/v1'); // http://bachman.xxx.com/scm/supplier/api/export/v1
 *    // 外部地址
...
export function download(pathname, param = {}, onerror = () => {}, method = 'post') {
  ...
  $iframe.onload = () => {
    try {
      ...
      const msg = html.match(reErrorMsg)[1];
      onerror({ msg });
    } catch (e) {
      onerror({ msg: '导出失败' });
    }
...
function processCallback(args) {
  ...
  if (typeof fn === 'function') {
    ...
    const uuid = param.router_callback_id = B.uuid();
    B[uuid] = fn;
  }
}
...
export function buildDownloadUrl(param) {
  return encodeURIComponent(param).replace(/%26/g, '&').replace(/%3D/g, '=');
}
